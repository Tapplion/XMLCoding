...
open class XMLDecoder {
    ...
    public enum KeyDecodingStrategy {
        ...
        ///
        /// Converting from Pascal case to camel case:
        /// 1. Lowercase the first letter
        /// For example, `OneTwoTree` becomes `oneTwoTree`.
        ///
        /// - Note: Using a key decoding strategy has a nominal performance cost, as each string key has to be processed.
        ...
        internal static func _convertFromPascalCase(_ stringKey: String) -> String {
            guard !stringKey.isEmpty else { return stringKey }
            let result = stringKey.prefix(1).lowercased() + stringKey.dropFirst()
            return result
        }
    ...
    /// Settings to be used by the XMLParser.
    open var xmlParserSettings: XMLParserSettings = XMLParserSettings()
    ...
    open func decode<T : Decodable>(_ type: T.Type, from data: Data) throws -> T {
        ...
        func parseAndFlatten(data: Data, withSettings settings: XMLParserSettings) throws -> [String: Any] {
            var result: [String: Any] = [:]
            
            var xmlOptions = XMLOptions()
            xmlOptions.parserSettings = xmlParserSettings
            
            guard let xmlDocument = try? XMLDocument(xml: data, options: xmlOptions) else {
                throw XMLError.parsingFailed
            }
            
            let rootElement = xmlDocument.root
            let childrenNames = rootElement.children.map{ $0.name }
            let set = NSSet(array: childrenNames)
            
            if set.count == 1 {
                result[rootElement.name] = rootElement.children.map{ $0.flattened }
            } else {
                result = xmlDocument.flattened
            }
        ...
        do {
            topLevel = try parseAndFlatten(data: data, withSettings: xmlParserSettings)
        ...
        if typeString.contains("Array") {
            let decodeType = [String: T].self
            
            guard let decoded = try decoder.unbox(topLevel, as: decodeType), let value = decoded.values.first else {
